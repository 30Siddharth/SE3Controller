...
t_final = 30
# P= np.array([[m.cos(m.pi/10),m.sin(m.pi/10),0],[-m.sin(m.pi/10),m.cos(m.pi/10),0],[0,0,1]]) 
P= np.array([[m.cos(m.pi/10),0,m.sin(m.pi/10)],[0,1,0],[-m.sin(m.pi/10),0,m.cos(m.pi/10)]])
Q =  R.from_matrix(P).as_quat()
...
# (fig, axes) = plt.subplots(nrows=2, ncols=1, sharex=True, num='DesPosition vs Time')
# x = state['x']
# x_des = flat['x']
# ax = axes[0]
# ax.plot(time, x_des[:,0], 'r', time, x_des[:,1], 'g', time, x_des[:,2], 'b')
...
M = R.from_quat(state['q']).as_dcm()
animate(time, state['x'], M, world=world, filename=None)
